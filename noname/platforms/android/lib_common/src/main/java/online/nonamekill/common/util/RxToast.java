...
public class RxToast {
    ...
    public static void showToast(Context context, String str, boolean isLong) {
        if(Objects.nonNull(getCurrentToast)){
            getCurrentToast.cancel();
            getCurrentToast = null;
        }
        if (isLong) {
            getCurrentToast = Toast.makeText(context, str, Toast.LENGTH_LONG);
        } else {
            getCurrentToast = Toast.makeText(context, str, Toast.LENGTH_SHORT);
        }
        getCurrentToast.show();
    }
